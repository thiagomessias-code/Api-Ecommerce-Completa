    ...
    async searchPedidos(req, res, next){
        ...
        try {
            ...
            const pedidos = await Pedido.paginate(
                ...
                {offset, limit, populate: ["clientes", "pagamento", "entrega"]}                
            );
...
//Put admin/:id
    ...
    async store(req, res, next){
        ...
        const cliente = new Cliente({nome, cpf, loja, email, telefones, endereco, dataDeNascimento, usuario: usuario._id});
    ...
    async update(req, res, next){
       ...
       try {
        ...
        if(!cliente) return res.send({error:"Não existe cliente"});
        if(nome){
            cliente.usuario.nome = nome;
            cliente.nome = nome;
        }
